...
export const suggestNewTripTool = tool(
  ...
  },
  {
    ...
    description:
      "Present a new trip draft card to the user. Use this when a trip is completed or cancelled and the user wants to start planning a new one. Pass any known context (destination, dates, title) or null for undecided fields. The card renders a visual widget with a button to create the trip. After calling this, respond conversationally — acknowledge what the user said, and let them know they can click the card to get started. Do not repeat the card's exact fields.",
    schema,
  },
);
